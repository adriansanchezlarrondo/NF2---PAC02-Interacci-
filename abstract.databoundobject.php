...
abstract class DataBoundObject {
   ...
   public function Load() {
      ...
      if (isset($this->ID)) {
        ...
        if($this->strTableName == 'login'){
            $strQuery .= " FROM \"" . $this->strTableName . "\" WHERE \"user_id\" = :eid";
        } else if ($this->strTableName == 'login_details'){
            $strQuery .= " FROM \"" . $this->strTableName . "\" WHERE \"login_details_id\" = :eid";
        } else {
            $strQuery .= " FROM \"" . $this->strTableName . "\" WHERE \"chat_message_id\" = :eid";
        }
   ...
   public function __destruct() {
      if (isset($this->ID)) {   
         if ($this->blForDeletion == true) {
            $strQuery = 'DELETE FROM "' . $this->strTableName . '"';
            if($this->strTableName == 'login'){
               $strQuery .= " WHERE \"user_id\" = :eid";
            } else if ($this->strTableName == 'login_details'){
               $strQuery .= " WHERE \"login_details_id\" = :eid";
